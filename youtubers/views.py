...
def search(request):
    tubers = Youtuber.objects.order_by('-created_date')
    city_search = Youtuber.objects.all().values_list('city', flat=True).distinct()
    camera_type_search = Youtuber.objects.all().values_list('camera_type', flat=True).distinct()
    category_search = Youtuber.objects.all().values_list('category', flat=True).distinct()
